...
use bluez_generated::{OrgBluezAdapter1Properties, OrgBluezDevice1Properties, OrgBluezGattCharacteristic1Properties, ORG_BLUEZ_ADAPTER1_NAME, ORG_BLUEZ_DEVICE1_NAME, ORG_BLUEZ_GATT_CHARACTERISTIC1_NAME};
use dbus::message::{MatchRule, SignalArgs};
use dbus::nonblock::stdintf::org_freedesktop_dbus::{ObjectManagerInterfacesAdded, PropertiesPropertiesChanged};
use dbus::{Message, Path};
use std::collections::HashMap;

use super::device::convert_manufacturer_data;
use super::{adapter::AdapterId, characteristic::CharacteristicId, device::DeviceId};
...
impl BluetoothEvent {
    ...
    pub(crate) fn match_rules(object: Option<impl Into<Path<'static>>>) -> Vec<MatchRule<'static>> {
        ...
        if object.is_none() {
            let match_rule = ObjectManagerInterfacesAdded::match_rule(Some(&bus_name), None).static_clone();
        ...
        let mut match_rule = PropertiesPropertiesChanged::match_rule(Some(&bus_name), object_path.as_ref()).static_clone();
    ...
    fn interfaces_added_to_events(interfaces_added: ObjectManagerInterfacesAdded) -> Vec<BluetoothEvent> {
        log::trace!("InterfacesAdded: {:?}", interfaces_added);
        let mut events = vec![];
        let object_path = interfaces_added.object;
        if let Some(_device) = OrgBluezDevice1Properties::from_interfaces(&interfaces_added.interfaces) {
            let id = DeviceId { object_path };
            events.push(BluetoothEvent::Device { id, event: DeviceEvent::Discovered })
        }
        events
    }

    /// Return a list of Bluetooth events parsed from a PropertiesChanged signal.
    fn properties_changed_to_events(object_path: Path<'static>, properties_changed: PropertiesPropertiesChanged) -> Vec<BluetoothEvent> {
        log::trace!("PropertiesChanged for {}: {:?}", object_path, properties_changed);
        let mut events = vec![];
        ...
        match properties_changed.interface_name.as_ref() {
            ...
            ORG_BLUEZ_DEVICE1_NAME => {
                ...
                if device.services_resolved() == Some(true) {
                    events.push(BluetoothEvent::Device { id, event: DeviceEvent::ServicesResolved });
                }
            }
            ORG_BLUEZ_GATT_CHARACTERISTIC1_NAME => {
                ...
                if let Some(value) = characteristic.value() {
                    events.push(BluetoothEvent::Characteristic {
                        id,
                        event: CharacteristicEvent::Value { value: value.to_owned() },
                    })
                }
            }
            _ => {}
...
#[cfg(test)]
mod tests {
    use super::super::service::ServiceId;
    ...
    fn device_rssi() {
        ...
        assert_eq!(BluetoothEvent::message_to_events(message), vec![BluetoothEvent::Device { id, event: DeviceEvent::RSSI { rssi } }])
    }
    ...
    fn device_manufacturer_data() {
        ...
        let message = device_manufacturer_data_message("/org/bluez/hci0/dev_11_22_33_44_55_66", manufacturer_data.clone());
        let id = DeviceId::new("/org/bluez/hci0/dev_11_22_33_44_55_66");
        assert_eq!(
            BluetoothEvent::message_to_events(message),
    ...
    fn characteristic_value() {
        ...
        let message = characteristic_value_message("/org/bluez/hci0/dev_11_22_33_44_55_66/service0012/char0034", &value);
        let id = CharacteristicId::new("/org/bluez/hci0/dev_11_22_33_44_55_66/service0012/char0034");
        assert_eq!(BluetoothEvent::message_to_events(message), vec![BluetoothEvent::Characteristic { id, event: CharacteristicEvent::Value { value } }])
    }
    ...
    fn device_discovered() {
        ...
        let id = DeviceId::new("/org/bluez/hci0/dev_11_22_33_44_55_66");
        assert_eq!(BluetoothEvent::message_to_events(message), vec![BluetoothEvent::Device { id, event: DeviceEvent::Discovered }])
    }
    ...
    fn match_rules_all() {
        ...
        let message = characteristic_value_message("/org/bluez/hci0/dev_11_22_33_44_55_66/service0012/char0034", &vec![1, 2, 3]);
        assert_eq!(match_rules.iter().any(|rule| rule.matches(&message)), true);
    }
    ...
    fn new_device_message(device_path: &'static str) -> Message {
        ...
        let interfaces_added = ObjectManagerInterfacesAdded { object: device_path.into(), interfaces };
    ...
    fn device_manufacturer_data_message(device_path: &'static str, manufacturer_data: HashMap<u16, Vec<u8>>) -> Message {
        let manufacturer_data: HashMap<_, _> = manufacturer_data.into_iter().map::<(u16, Variant<Box<dyn RefArg>>), _>(|(k, v)| (k, Variant(Box::new(v)))).collect();
        let mut changed_properties: HashMap<String, Variant<Box<dyn RefArg>>> = HashMap::new();
        changed_properties.insert("ManufacturerData".to_string(), Variant(Box::new(manufacturer_data)));
        let properties_changed = PropertiesPropertiesChanged {
            interface_name: "org.bluez.Device1".to_string(),
            changed_properties,
            invalidated_properties: vec![],
