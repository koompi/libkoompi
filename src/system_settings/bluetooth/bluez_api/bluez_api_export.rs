pub use super::adapter::{AdapterId, AdapterInfo};
pub use super::bleuuid::{uuid_from_u16, uuid_from_u32, BleUuid};
pub use super::characteristic::{CharacteristicFlags, CharacteristicId, CharacteristicInfo};
pub use super::descriptor::{DescriptorId, DescriptorInfo};
pub use super::device::{AddressType, DeviceId, DeviceInfo};
pub use super::events::{AdapterEvent, BluetoothEvent, CharacteristicEvent, DeviceEvent};
use super::introspect::IntrospectParse;
use super::messagestream::MessageStream;
pub use super::service::{ServiceId, ServiceInfo};
...
use crate::system_settings::bluetooth::bluez_generated;
use bluez_generated::{OrgBluezAdapter1, OrgBluezAdapter1Properties, OrgBluezDevice1, OrgBluezDevice1Properties, OrgBluezGattCharacteristic1, OrgBluezGattDescriptor1, OrgBluezGattService1, ORG_BLUEZ_ADAPTER1_NAME, ORG_BLUEZ_DEVICE1_NAME};
...
impl Into<PropMap> for &DiscoveryFilter {
    fn into(self) -> PropMap {
        ...
        if let Some(pathloss_threshold) = self.pathloss_threshold {
            map.insert("Pathloss".to_string(), Variant(Box::new(pathloss_threshold)));
        }
        if let Some(transport) = self.transport {
            map.insert("Transport".to_string(), Variant(Box::new(transport.to_string())));
        }
        if let Some(duplicate_data) = self.duplicate_data {
            map.insert("DuplicateData".to_string(), Variant(Box::new(duplicate_data)));
        }
...
impl Default for BluetoothSession {
    fn default() -> Self {
        let (_, async_connection) = dbus_tokio::connection::new_system_sync().unwrap();
...
impl BluetoothSession {
    ...
    pub async fn new() -> Result<(impl Future<Output = Result<(), SpawnError>>, Self), BluetoothError> {
        ...
        Ok((dbus_handle.map(|res| Ok(res??)), BluetoothSession { connection }))
    }
    ...
    pub async fn start_discovery(&self) -> Result<(), BluetoothError> {
        self.start_discovery_with_filter(&DiscoveryFilter::default()).await
    }
    ...
    pub async fn start_discovery_on_adapter(&self, adapter: &AdapterId) -> Result<(), BluetoothError> {
        self.start_discovery_on_adapter_with_filter(adapter, &DiscoveryFilter::default()).await
    }
    ...
    pub async fn start_discovery_with_filter(&self, discovery_filter: &DiscoveryFilter) -> Result<(), BluetoothError> {
        ...
        for adapter in adapters {
            ...
            self.start_discovery_on_adapter_with_filter(&adapter.id, discovery_filter).await.unwrap_or_else(|err| println!("starting discovery failed {:?}", err));
        }
        Ok(())
    }
    ...
    pub async fn start_discovery_on_adapter_with_filter(&self, adapter_id: &AdapterId, discovery_filter: &DiscoveryFilter) -> Result<(), BluetoothError> {
        ...
        adapter.set_discovery_filter(discovery_filter.into()).await?;
    ...
    pub async fn get_adapters(&self) -> Result<Vec<AdapterInfo>, BluetoothError> {
        let bluez_root = Proxy::new("org.bluez", "/", DBUS_METHOD_CALL_TIMEOUT, self.connection.clone());
    ...
    pub async fn get_devices_on_adapter(&self, adapter: &AdapterId) -> Result<Vec<DeviceInfo>, BluetoothError> {
        ...
        Ok(devices.into_iter().filter(|device| device.id.adapter() == *adapter).collect())
    }
    ...
    pub async fn get_descriptors(&self, characteristic: &CharacteristicId) -> Result<Vec<DescriptorInfo>, BluetoothError> {
        let characteristic_node = self.characteristic(characteristic).introspect_parse().await?;
    ...
    pub async fn get_service_by_uuid(&self, device: &DeviceId, uuid: Uuid) -> Result<ServiceInfo, BluetoothError> {
        ...
        services.into_iter().find(|service_info| service_info.uuid == uuid).ok_or(BluetoothError::UUIDNotFound { uuid })
    }
    ...
    pub async fn get_characteristic_by_uuid(&self, service: &ServiceId, uuid: Uuid) -> Result<CharacteristicInfo, BluetoothError> {
        ...
        characteristics.into_iter().find(|characteristic_info| characteristic_info.uuid == uuid).ok_or(BluetoothError::UUIDNotFound { uuid })
    }
    ...
    pub async fn get_service_characteristic_by_uuid(&self, device: &DeviceId, service_uuid: Uuid, characteristic_uuid: Uuid) -> Result<CharacteristicInfo, BluetoothError> {
        ...
        self.get_characteristic_by_uuid(&service.id, characteristic_uuid).await
    }
    ...
    pub async fn get_service_info(&self, id: &ServiceId) -> Result<ServiceInfo, BluetoothError> {
        ...
        let uuid = Uuid::parse_str(&service.uuid().await?)?;
        let primary = service.primary().await?;
        Ok(ServiceInfo { id: id.to_owned(), uuid, primary })
    }
    ...
    pub async fn get_descriptor_info(&self, id: &DescriptorId) -> Result<DescriptorInfo, BluetoothError> {
        ...
        Ok(DescriptorInfo { id: id.to_owned(), uuid })
    }

    fn adapter(&self, id: &AdapterId) -> impl OrgBluezAdapter1 + Introspectable + Properties {
        Proxy::new("org.bluez", id.object_path.to_owned(), DBUS_METHOD_CALL_TIMEOUT, self.connection.clone())
    }
    ...
    fn characteristic(&self, id: &CharacteristicId) -> impl OrgBluezGattCharacteristic1 + Introspectable + Properties {
        Proxy::new("org.bluez", id.object_path.to_owned(), DBUS_METHOD_CALL_TIMEOUT, self.connection.clone())
    }

    fn descriptor(&self, id: &DescriptorId) -> impl OrgBluezGattDescriptor1 + Introspectable + Properties {
        Proxy::new("org.bluez", id.object_path.to_owned(), DBUS_METHOD_CALL_TIMEOUT, self.connection.clone())
    }
    ...
    pub async fn write_characteristic_value(&self, id: &CharacteristicId, value: impl Into<Vec<u8>>) -> Result<(), BluetoothError> {
        self.write_characteristic_value_with_options(id, value, WriteOptions::default()).await
    }
    ...
    pub async fn write_characteristic_value_with_options(&self, id: &CharacteristicId, value: impl Into<Vec<u8>>, options: WriteOptions) -> Result<(), BluetoothError> {
        let characteristic = self.characteristic(id);
        Ok(characteristic.write_value(value.into(), options.into()).await?)
    }
    ...
    pub async fn write_descriptor_value_with_offset(&self, id: &DescriptorId, value: impl Into<Vec<u8>>, offset: usize) -> Result<(), BluetoothError> {
        let descriptor = self.descriptor(id);
        Ok(descriptor.write_value(value.into(), offset_to_propmap(offset)).await?)
    }
    ...
    async fn filtered_event_stream(&self, object: Option<&(impl Into<Path<'static>> + Clone)>) -> Result<impl Stream<Item = BluetoothEvent>, BluetoothError> {
        ...
        Ok(select_all(message_streams).flat_map(|message| stream::iter(BluetoothEvent::message_to_events(message))))
    }
}
