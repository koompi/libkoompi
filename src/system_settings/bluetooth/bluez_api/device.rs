...
use super::{
    adapter::AdapterId,
    bluez_api_export::{BluetoothError, MacAddress},
};
...
impl DeviceId {
    pub(crate) fn new(object_path: &str) -> Self {
        Self { object_path: object_path.to_owned().into() }
    }

    /// Get the ID of the Bluetooth adapter on which this device was discovered, e.g. `"hci0"`.
    pub fn adapter(&self) -> AdapterId {
        let index = self.object_path.rfind('/').expect("DeviceId object_path must contain a slash.");
...
impl Display for DeviceId {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.object_path.to_string().strip_prefix("/org/bluez/").ok_or(fmt::Error)?)
    }
}
...
impl DeviceInfo {
    pub(crate) fn from_properties(id: DeviceId, device_properties: OrgBluezDevice1Properties) -> Result<DeviceInfo, BluetoothError> {
        let mac_address = device_properties.address().ok_or_else(|| BluetoothError::RequiredPropertyMissing("Address"))?;
        let address_type = device_properties.address_type().ok_or_else(|| BluetoothError::RequiredPropertyMissing("AddressType"))?.parse()?;
        ...
        Ok(DeviceInfo {
            ...
            services,
            paired: device_properties.paired().ok_or_else(|| BluetoothError::RequiredPropertyMissing("Paired"))?,
            connected: device_properties.connected().ok_or_else(|| BluetoothError::RequiredPropertyMissing("Connected"))?,
            rssi: device_properties.rssi(),
            tx_power: device_properties.tx_power(),
            icon: device_properties.icon().map(|x| x.to_owned()),
            manufacturer_data,
            service_data,
            services_resolved: device_properties.services_resolved().ok_or_else(|| BluetoothError::RequiredPropertyMissing("ServicesResolved"))?,
        })
    }
}
...
pub(crate) fn convert_manufacturer_data(data: &HashMap<u16, Variant<Box<dyn RefArg>>>) -> HashMap<u16, Vec<u8>> {
    data.iter()
        .filter_map(|(&k, v)| {
...
#[cfg(test)]
mod tests {
    use super::super::bleuuid::uuid_from_u32;
    use super::*;
    #[test]
    fn device_adapter() {
    ...
    fn service_data() {
        ...
        assert_eq!(get_service_data(OrgBluezDevice1Properties(&device_properties)), Some(expected_service_data));
    }
    ...
    fn manufacturer_data() {
        ...
        let mut device_properties: HashMap<String, Variant<Box<dyn RefArg>>> = HashMap::new();
        device_properties.insert("ManufacturerData".to_string(), Variant(Box::new(manufacturer_data)));

        let mut expected_manufacturer_data = HashMap::new();
        expected_manufacturer_data.insert(manufacturer_id, vec![1u8, 2, 3]);

        assert_eq!(get_manufacturer_data(OrgBluezDevice1Properties(&device_properties)), Some(expected_manufacturer_data));
    }
    ...
    fn device_info_minimal() {
        ...
        let mut device_properties: HashMap<String, Variant<Box<dyn RefArg>>> = HashMap::new();
        device_properties.insert("Address".to_string(), Variant(Box::new("00:11:22:33:44:55".to_string())));
        device_properties.insert("AddressType".to_string(), Variant(Box::new("public".to_string())));
        ...
        let device = DeviceInfo::from_properties(id.clone(), OrgBluezDevice1Properties(&device_properties)).unwrap();
        assert_eq!(
            device,
            DeviceInfo {
                id,
    ...
    fn get_services_none() {
        ...
        assert_eq!(get_services(OrgBluezDevice1Properties(&device_properties)), vec![])
    }
    ...
    fn get_services_some() {
        ...
        assert_eq!(get_services(OrgBluezDevice1Properties(&device_properties)), vec![uuid])
    }
