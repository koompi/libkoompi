...
pub trait OrgBluezHealthManager1 {
    fn create_application(&self, config: arg::PropMap) -> nonblock::MethodReply<dbus::Path<'static>>;
...
impl<'a, T: nonblock::NonblockReply, C: ::std::ops::Deref<Target = T>> OrgBluezHealthManager1 for nonblock::Proxy<'a, C> {
    fn create_application(&self, config: arg::PropMap) -> nonblock::MethodReply<dbus::Path<'static>> {
        self.method_call("org.bluez.HealthManager1", "CreateApplication", (config,)).and_then(|r: (dbus::Path<'static>,)| Ok(r.0))
    }
