...
use super::dbus_api::{extract, path_to_string, variant_iter_to_vec_u8, DBusApi, VariantTo};
use super::errors::*;
use super::ssid::{AsSsidSlice, Ssid};
use super::types::ConnectionState;
use super::types::{Connectivity, DeviceState, DeviceType, NetworkManagerState};
use super::wifi::{AccessPoint, AccessPointCredentials, NM80211ApFlags, NM80211ApSecurityFlags};
use ascii::AsciiStr;
...
fn verify_ascii_password(password: &str) -> Result<&str> {
    match AsciiStr::from_ascii(password) {
        ...
        Ok(p) => {
            if p.len() < 8 {
                Err(ErrorKind::PreSharedKey(format!(
