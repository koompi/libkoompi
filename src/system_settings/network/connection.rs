...
use super::device::{get_active_connection_devices, Device};
use super::ssid::{AsSsidSlice, Ssid};
use super::types::ConnectionState;
use super::wifi::{AccessPoint, AccessPointCredentials};
use log::debug;
#[derive(Clone)]
    ...
    pub fn activate(&self) -> Result<ConnectionState> {
        ...
        match state {
            ...
            ConnectionState::Unknown => {
                Err(ErrorKind::NetworkManager("Unable to get connection state".to_string()).into())
            }
            _ => {
