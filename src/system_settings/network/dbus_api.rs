// what we have refactored for this file was:
/*
1. rename all imported dbus library to be up-to-date.
2. import failure library
3. change DBusConnection::get_private(BusTyep::System).unwrap() to DBusConnection::new_system().unwrap();
4. &RefArg -> & dyn RefArg for the sake of object trait feature in rust.

*/
use super::errors::*;
use dbus::arg::{Array, Get, Iter, RefArg, Variant};
use dbus::ffidisp::stdintf::OrgFreedesktopDBusProperties;
use dbus::ffidisp::ConnPath;
...
impl DBusApi {
    pub fn new(
        ...
        let connection = DBusConnection::new_system().unwrap();
            ...
            Err(e) => {
                {
                    ...
                    for error_name in self.method_retry_error_names {
                        if name == Some(error_name) {
                            log::debug!("Should retry D-Bus method call: {}", error_name);
        ...
        DBusApi: VariantTo<T>,
    {
        ...
        match path.get(interface, name) {
            Ok(variant) => match DBusApi::variant_to(&variant) {
                Some(data) => Ok(data),
                None => Err(ErrorKind::DBusAPI("property wrong".to_string()).into()),
            },
            Err(e) => {
        ...
        T2: Get<'a>,
    {
        ...
        Err(ErrorKind::DBusAPI("Wrong response type".to_string()).into())
    }
...
pub fn variant_iter_to_vec_u8(var: &mut Variant<Iter>) -> Result<Vec<u8>> {
    ...
    if let Some(array) = *array_option {
        ...
        Err(ErrorKind::DBusAPI(format!("Variant not an array: {:?}", var)).into())
    }
}
